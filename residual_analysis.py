...
def fitted_and_residuals(X, y, beta_hat):
    ...
    if len(y) != n:
        raise ValueError("len(y) phải bằng số dòng của X")

    if len(beta_hat) != p:
        raise ValueError("len(beta_hat) phải bằng số cột của X")
...
def leverages(X):
    """Trả về list các leverage h_ii."""
    H, _ = hat_matrix(X)
    return [H[i][i] for i in range(len(H))]


def standardized_residuals(e, h, sigma2):
    """r_i = e_i / (sigma_hat * sqrt(1 - h_i))."""

    if sigma2 <= 1e-12:
        raise ValueError(
            "sigma2 gần bằng 0. Mô hình fit gần như hoàn hảo, "
            "không thể chuẩn hóa residual ổn định."
        )
    ...
    for i in range(len(e)):
        ...
        denom = sig * math.sqrt(one_minus_h)
...
def cooks_distance(r, h, p):
    ...
    if p <= 0:
        raise ValueError("p phải > 0 để tính Cook's distance")
    ...
    for i in range(len(r)):
        ...
        out.append((r[i] ** 2 / p) * h[i] / one_minus_h)
...
def norm_ppf(p):
    """
    Inverse CDF của N(0, 1) dùng Acklam's approximation.
    Sai số tuyệt đối xấp xỉ 1.15e-9.
    """

    if not (0.0 < p < 1.0):
        raise ValueError("norm_ppf: p phải nằm trong (0, 1)")
...
def residual_plots(X, y, beta_hat, savepath="residual_plots.png"):
    """
    Vẽ 4 biểu đồ phân tích phần dư cho hồi quy tuyến tính:

    1. Residuals vs Fitted
    2. Normal Q-Q
    3. Scale-Location
    4. Residuals vs Leverage, kèm Cook's distance

    Các đại lượng e, h, sigma2, standardized residuals và Cook's D
    được tính thủ công, không dùng numpy.
    """
    ...
    if p == 0:
        raise ValueError("X phải có ít nhất 1 cột")
    ...
    if n <= p:
        raise ValueError(
            "Cần n > p để tính sigma2 = RSS / (n - p). "
            "Hiện tại n <= p nên diagnostics không ổn định."
        )
    ...
    try:
        ...
        raise ValueError(
            "Không thể tính hat matrix. Có thể X^T X bị suy biến "
            "do các cột của X phụ thuộc tuyến tính."
        ) from err
    ...
    lo = min(theo[0], e_sorted[0])
    hi = max(theo[-1], e_sorted[-1])

    ax.plot([lo, hi], [lo, hi], color="red", linewidth=1, linestyle="--")
    ax.set_xlabel("Quantile lý thuyết N(0,1)")
    ax.set_ylabel("Quantile của residual chuẩn hóa")
    ...
    h_grid = [0.001 + 0.001 * k for k in range(999)]
    ...
    max_h = max(h)
    ax.set_xlim(0, min(1.0, max_h * 1.1 + 1e-6))

    rmax = max(abs(min(r)), abs(max(r))) * 1.2 + 1.0
    ...
    if high_leverage_points:
        print(
            "[Cảnh báo] Một số điểm có leverage cao:",
            high_leverage_points,
        )
...
def simulate_with_outliers(n=80, seed=7):
    """
    Tạo dữ liệu giả lập có:
    - nhiễu heteroscedasticity nhẹ
    - một điểm leverage cao
    - một điểm outlier theo y
    """

    if n < 3:
        raise ValueError("n phải >= 3 vì hàm thêm 2 điểm outlier riêng")
    ...
    X.append([1.0, 6.0, 6.0])
    y.append(20.0)

    X.append([1.0, 0.5, -0.3])
    y.append(15.0)
...
def verify_with_statsmodels(X, y, mine):
    """
    Kiểm chứng leverage và Cook's distance với statsmodels.
    Phần này chỉ chạy nếu có numpy và statsmodels.
    """
    ...
    try:
        ...
        print(
            "[Bỏ qua kiểm chứng] statsmodels không có sẵn. "
            "Có thể cài bằng: pip install statsmodels"
        )
        return
    ...
    h_diff = max(
        abs(mine["leverage"][i] - h_sm[i])
        for i in range(n)
    )

    cook_diff = max(
        abs(mine["cooks"][i] - cook_sm[i])
        for i in range(n)
    )

    print("\n--- Kiểm chứng leverage & Cook's với statsmodels ---")
